...
### Dataset: offline аугментация 84 → 2,455 сэмплов
`scripts/augment_dataset.py` — детерминированная слот-аугментация (seed 42, без LLM-API): spam = продукт × оффер × urgency × канал, toxic = 28 паттернов оскорблений × интенсификатор × таргет, safe = приветствия/вопросы/комментарии по 20 темам. Дедупликация по lower() (вычистила 88 дублей из исходных 172 строк).

| | До | После |
|---|---|---|
| Уникальных сэмплов | 84 | 2,455 |
| Test accuracy / F1-macro | 0.87 | **0.994** |
| Sanity на живых фразах вне шаблонов | 4/8 | **8/10** |

Замечание о честности метрик: шаблонная аугментация завышает holdout-метрики (вариации одного шаблона попадают в train и test), поэтому решающей проверкой выбран sanity-набор фраз, не встречавшихся при генерации. Пограничные кейсы («waste of time» без адресата) отдаются guardrails-слою с эвристиками.
